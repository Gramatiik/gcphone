...
--  Contacts
--====================================================================================
function getContactsByIdentifier(identifier)
    local result = MySQL.Sync.fetchAll("SELECT id, phone_number, name FROM phone_contacts WHERE identifier = @identifier", {
        ['@identifier'] = identifier
    })
    print(result)
    return result
...
function addContact(source, identifier, phone_number, name)
    MySQL.Sync.execute("INSERT INTO phone_contacts (`identifier`, `phone_number`,`name`) VALUES(@identifier, @phone_number, @name)", {
        ['@identifier'] = identifier,
        ['@phone_number'] = phone_number,
        ['@name'] = name,
    })
    notifyContactsChanged(source, identifier)
end

function updateContact(source, identifier, id, phone_number, name)
    MySQL.Sync.execute("UPDATE phone_contacts SET phone_number = @phone_number, name = @name WHERE id = @id", {
        ['@phone_number'] = phone_number,
        ['@name'] = name,
        ['@id'] = id,
    })
...
function notifyContactsChanged(source, identifier)
    if source ~= nil then
        TriggerClientEvent("gcPhone:contactList", source, getContactsByIdentifier(identifier))
    end
end

RegisterServerEvent('gcPhone:addContact')
AddEventHandler('gcPhone:addContact', function(name, phoneNumber)
    local identifier = GetPlayerIdentifiers(source)[1]
    addContact(source, identifier, phoneNumber, name)
end)

RegisterServerEvent('gcPhone:updateContact')
AddEventHandler('gcPhone:updateContact', function(id, name, phoneNumber)
    local identifier = GetPlayerIdentifiers(source)[1]
    updateContact(source, identifier, id, phoneNumber, name)
end)
...
function _internalAddMessage(transmitter, receiver, message)
    ...
    local Query = "INSERT INTO phone_messages (`from`, `to`,`content`, `isRead`) VALUES(@transmitter, @receiver, @message, @isRead)"
...
function addMessage(source, identifier, phone_number, message)
    ...
    local myPhone = getPhoneNumberByIdentifier(identifier)
...
function setReadMessageNumber(identifier, num)
    local mePhoneNumber = getPhoneNumberByIdentifier(identifier)
...
AddEventHandler('es:playerLoaded',function(source)
    ...
    local myPhoneNumber = getPhoneNumberByIdentifier(identifier)
    if myPhoneNumber == nil or myPhoneNumber == 0 then
        MySQL.Sync.execute("UPDATE users SET phone_number = CRC32(@identifier) WHERE identifier = @identifier", {
            ['@identifier'] = identifier
        })
        myPhoneNumber = getPhoneNumberByIdentifier(identifier)
    end
    TriggerClientEvent("gcPhone:myPhoneNumber", _source, myPhoneNumber)
    TriggerClientEvent("gcPhone:contactList", _source, getContactsByIdentifier(identifier))
...
AddEventHandler('gcPhone:allUpdate', function()
    local identifier = GetPlayerIdentifiers(source)[1]
    TriggerClientEvent("gcPhone:myPhoneNumber", source, getPhoneNumberByIdentifier(identifier))
    TriggerClientEvent("gcPhone:contactList", source, getContactsByIdentifier(identifier))
    --TriggerClientEvent("gcPhone:allMessage", source, getMessages(identifier))
end)
...
-- print('Phone number: ' .. getPhoneNumberByIdentifier(identifier))

-- print('')
-- print('Contacts')
-- local listContacts = getContactsByIdentifier(identifier)
